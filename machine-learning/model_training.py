...
"""
path = os.path.join(os.getcwd(), "processed_data", "more_data.csv")
...
retweets_features = np.concatenate((data[:, :-2], favorites), axis=1)

# Splitting train data and test data
# Retweets Data Split
retweets_features_train, retweets_features_test, retweets_train, retweets_test = train_test_split(retweets_features, retweets)
...
favorites_features_train, favorites_features_test, favorites_train, favorites_test = train_test_split(features, favorites)
 # Algorithm 
retweet_linear_regression = Ridge(max_iter=100000, normalize=True)
favorites_linear_regression = Lasso(max_iter=10000, normalize=True)
